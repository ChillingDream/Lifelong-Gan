...
for e in range(epoch):
    for idx in range(train_A):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss_vae_D=(tf.reduce_mean(tf.math.squared_difference(P_real,0.9))+tf.reduce_mean(tf.square(P_fake_encoded)))
            loss_lr_D=(tf.reduce_mean(tf.math.squared_difference(P_real,0.9))+tf.reduce_mean(tf.square(P_fake)))	
            loss_vae_GE=tf.reduce_mean(tf.math.squared_difference(P_fake_encoded,0.9))
            ...
            loss_kl_E=0.5*tf.reduce_mean(-1-encoded_log_sigma+encoded_mu**2+tf.exp(encoded_log_sigma))
        
            loss_D=loss_vae_D+loss_lr_D-tf.reduce_mean(tf.math.squared_difference(P_real, 0.9))
    ...
    if e!=0 and e%10==0:
        tl.vis.save_images(desired_gen_img.numpy(),[2,4],os.path.join(save_dir,'vae_g_{}.png'.format(e)))
        tl.vis.save_images(LR_desired_img.numpy(),[2,4],os.path.join(save_dir,'lr_g_{}.png'.format(e)))
